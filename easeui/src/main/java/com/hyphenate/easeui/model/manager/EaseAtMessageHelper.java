...
public class EaseAtMessageHelper {
    ...
    public synchronized static EaseAtMessageHelper getInstance(){
        if(instance == null){
    ...
    public boolean containsAtAll(String content){
        String atAll = "@" + EaseUIHelper.getInstance().getContext().getString(R.string.all_members);
    ...
    public void parseMessages(List<EMMessage> messages) {
        ...
        for(EMMessage msg : msgs){
            if(msg.getChatType() == EMMessage.ChatType.GroupChat){
                ...
                try {
                    JSONArray jsonArray = msg.getJSONArrayAttribute(EaseUIConstant.MESSAGE_ATTR_AT_MSG);
                    ...
                    String usernameStr = msg.getStringAttribute(EaseUIConstant.MESSAGE_ATTR_AT_MSG, null);
                    if(usernameStr != null){
                        if(usernameStr.toUpperCase().equals(EaseUIConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                            if(!atMeGroupList.contains(groupId)){
                                atMeGroupList.add(groupId);
                            }
                        }
    ...
    public boolean isAtMeMsg(EMMessage message){
        ...
        if(user != null){
            try {
                JSONArray jsonArray = message.getJSONArrayAttribute(EaseUIConstant.MESSAGE_ATTR_AT_MSG);
                ...
                String atUsername = message.getStringAttribute(EaseUIConstant.MESSAGE_ATTR_AT_MSG, null);
                if(atUsername != null){
                    if(atUsername.toUpperCase().equals(EaseUIConstant.MESSAGE_ATTR_VALUE_AT_MSG_ALL)){
                        return true;
                    }
                }
                return  false;
